...
struct AccessIteratorFalsePredicate {
    bool operator()(const AccessRecord&) { return false; }
};

template <typename SkipAccessPredicate = AccessIteratorFalsePredicate> struct AccessIterator {
    ...
    explicit operator bool() const { return position < end; }
};
